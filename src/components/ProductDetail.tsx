...
export default function ProductDetail({ product }: { product: Product }) {
  ...
  return (
    <BackdropSection as="main" variant={1} className="px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      <article className="ornate-frame paper-panel mx-auto max-w-5xl rounded-[30px] p-5 shadow-lotus sm:p-8 lg:p-12">
        <div className="text-center">
          ...
          <h1 className="mt-5 font-serif text-4xl font-bold leading-tight text-lotusGreen sm:text-5xl lg:text-6xl">{product.name}</h1>
        ...
        <section className="mx-auto mt-8 max-w-3xl">
          <p className="text-center text-xs font-bold uppercase tracking-[0.2em] text-lotusGold sm:text-sm sm:tracking-[0.26em]">Tờ giới thiệu sản phẩm</p>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          {[
            ...
            <div key={label} className="rounded-[20px] border border-borderGold/45 bg-lightCream/75 p-5">
              ...
              <p className="mt-2 text-sm leading-7 text-textDarkGreen/76 sm:text-base">{value}</p>
            </div>
          ))}
        </div>
      ...
      <section className="mx-auto mt-14 max-w-7xl">
        <h2 className="text-center font-serif text-3xl font-bold text-lotusGreen sm:text-4xl">Sản phẩm liên quan</h2>
